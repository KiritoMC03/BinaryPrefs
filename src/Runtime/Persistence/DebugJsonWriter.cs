    ...
    internal static class DebugJsonWriter
    {
        ...
        private static IEnumerable<KeyValuePair<string, object?>> ProjectRoot(IReadOnlyDictionary<string, Record> data)
        {
            foreach (var pair in data)
                yield return new KeyValuePair<string, object?>(pair.Key ?? "null", pair.Value?.Object);
        }
        ...
        )
        {
            switch (value)
            {
                ...
                case Vector2 vector2:
                    WriteEntries(
                        builder,
                        Members(("x", vector2.x), ("y", vector2.y)),
                        depth,
                        visited
                    );
                    return;
                case Vector3 vector3:
                    WriteEntries(
                        builder,
                        Members(("x", vector3.x), ("y", vector3.y), ("z", vector3.z)),
                        depth,
                        visited
                    );
                    return;
                ...
                case Vector2Int vector2Int:
                    WriteEntries(
                        builder,
                        Members(("x", vector2Int.x), ("y", vector2Int.y)),
                        depth,
                        visited
                    );
                    return;
                case Vector3Int vector3Int:
                    WriteEntries(
                        builder,
                        Members(("x", vector3Int.x), ("y", vector3Int.y), ("z", vector3Int.z)),
                        depth,
                        visited
                    );
                    return;
        ...
        private static void WriteReflected
        (
            StringBuilder builder,
            object value,
            int depth,
        ...
        private static bool TryReadMember(Func<object> getter, [NotNullWhen(true)] out object? result)
        {
            try
            {
                result = getter();
        ...
        private static void WriteFloating
        (
            StringBuilder builder,
            object value,
            bool isSpecial,
        ...
        private static KeyValuePair<string, object?>[] Members(params (string Key, object Value)[] items)
        {
            ...
            for (var i = 0; i < items.Length; i++)
                members[i] = new KeyValuePair<string, object?>(items[i].Key ?? "null", items[i].Value);
            return members;
        }
