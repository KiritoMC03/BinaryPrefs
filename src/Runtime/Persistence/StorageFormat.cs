...
namespace Appegy.Storage
{
    internal static class StorageFormat
    {
        ...
        internal static void Write
            (BinaryWriter writer, IReadOnlyList<BinarySection> sections, Dictionary<string, Record> data)
        {
            ...
            foreach (var entry in data)
            {
                ...
                {
                    Debug.LogError(
                        $"[BinaryStorage] Null key. ({entry.Value?.Type.FullName} | {entry.Value?.TypeIndex})"
                    );
                    continue;
                }
                if (entry.Value == null)
                {
                    Debug.LogError($"Null value for {entry.Key}");
                    continue;
                }
                var section = sections[entry.Value.TypeIndex];
                if (section == null)
                {
                    Debug.LogError($"No section at index {entry.Value.TypeIndex}");
                    continue;
                }
                ...
                section.WriteTo(writer, entry.Value);
        ...
        private static FileSection[] ReadHeader
        (
            string storageFilePath,
            IReadOnlyList<BinarySection> sections,
            BinaryReader reader,
            ...
            try
            {
                ...
                if (serializersCount < 0 || serializersCount > stream.Length - stream.Position)
                    throw new StorageFileCorruptedException(
                        storageFilePath,
                        $"Invalid serializer count {serializersCount}"
                    );
                ...
                for (var i = 0; i < serializersCount; i++)
                {
                    ...
                    fileSections[i] = new FileSection(
                        typeName,
                        sectionIndex == -1 ? null : sections[sectionIndex],
                        sectionIndex
                    );
                }
        ...
        private static RecordHeader ReadRecordHeader(string storageFilePath, BinaryReader reader, long fileLength)
        {
            ...
            catch (EndOfStreamException e)
            {
                throw new StorageFileCorruptedException(
                    storageFilePath,
                    "Unexpected end of file while reading record header",
                    e
                );
            }
            ...
            if (size < 0 || valuePosition + size > fileLength)
                throw new StorageFileCorruptedException(
                    storageFilePath,
                    $"Entry '{key}' of {size}b at {valuePosition} runs past the end of a {fileLength}b file"
                );
        ...
        )
        {
            if (header.TypeIndex < 0 || header.TypeIndex >= fileSections.Length)
            {
                ReportFailedKey(
                    ...
                    UnknownTypeName,
                    keyLoadFailedBehaviour,
                    $"Type index {header.TypeIndex} is out of range"
                );
                return false;
            }
            ...
            if (fileSection.Section == null)
            {
                ReportFailedKey(
                    header,
                    fileSection.TypeName,
                    keyLoadFailedBehaviour,
                    "Unregistered type serializer"
                );
                return false;
            }
            ...
            if (readSize != header.Size)
            {
                ReportFailedKey(
                    ...
                    fileSection.TypeName,
                    keyLoadFailedBehaviour,
                    $"Read more than expected ({readSize}b)"
                );
                return false;
            }
        ...
        private static void ReportFailedKey
        (
            in RecordHeader header,
            string typeName,
            KeyLoadFailedBehaviour keyLoadFailedBehaviour,
        ...
        )
        {
            switch (keyLoadFailedBehaviour)
            {
                ...
                case KeyLoadFailedBehaviour.IgnoreWithWarning:
                    Debug.LogWarning(
                        $"Failed to load key {header.Key} of type {typeName} with size {header.Size}b. Reason: {reason}"
                    );
                    break;
                default:
